...
- `dsh-skill-badge@0.1.0-rc.8` is the upstream seed entry. Its module top level builds immutable provider metadata and file URLs; it reads the bundled Markdown only when the skill is requested. `apply` registers one provider on the existing `skills` service, and Cordis removes that registration on disposal.
